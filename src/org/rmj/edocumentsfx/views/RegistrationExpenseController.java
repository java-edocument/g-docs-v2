...
public class RegistrationExpenseController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
         if (poGRider == null){
            ...
            if(isloaded == false){
                if (psDeptNm2.equals("")) psDeptNm2 = loRegistrationSelector.getDepartment();
                if(pnDocSize== 0) pnDocSize = loRegistrationSelector.getDocsSize();
            }
    ...
    private void TableClick(MouseEvent event) {
        ...
        if(event.getClickCount() ==1){
                ...
                try {
                    ...
                    Logger.getLogger(RegistrationExpenseController.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    }
    ...
    public void setRegistrationExpense(RegistrationExpenseSelectorController loController){
        this.loRegistrationSelector = loController;
    }
    
    public void setRegistrationExpenseRegister(RegistrationExpenseSelectorRegisterController loController){
    ...
    public void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            ...
            case "btnsearch":
                if (poTrans.getRegisExpense(txtField00.getText()) == true){
                if(poTrans.MasterFileCount()!= 0){
...
//                            poTrans.sample_fileMultiple();
            ...
            case "btncancel"://Exit
                if(ShowMessageFX.YesNo("All changes made including scanned images will be disregard?", pxeModuleName,"Do you want to close this form?" )==true){
                    ...
                    clearFields();
                    loRegistrationSelector.setStage();
                    unloadScene(event);
                }
                break;
            ...
            case "btnclose":
                if(isloaded ==false){
                    loRegistrationSelector.setStage();
                    loRegistrationSelector.clearFields();
                    loRegistrationSelector.initButton(EditMode.UNKNOWN);
                }else{
                    try{
...
//                }
            ...
            case "btnback":
                if(isloaded ==false){
                    loRegistrationSelector.setDeptName(psDeptNm2);
                    loRegistrationSelector.setDocSize(pnDocSize);
            ...
            switch (lnIndex){
                case 0:
                    if(event.getCode() == ENTER){
                        ...
                        if (poTrans.getRegisExpense(lsValue) == true){
                            if(poTrans.MasterFileCount()!= 0){
                                if(ShowMessageFX.YesNo("All scan images will be deleted. Continue?", pxeModuleName, "Scanned Docs detected!")==true){
                                    if(poTrans.deleteAllImage()==true){    
                                        ShowMessageFX.Information(null, pxeModuleName, "All images successfully deleted!");
    ...
    public void loadDetail2Grid() {
        ...
        for(lnCtr = 0; lnCtr <= dataSize -1 ; lnCtr++){
            items.add(new TableWithCheckBoxModel(String.valueOf(lnCtr+1),
                    ...
                    String.valueOf(poTrans.getDetail(lnCtr, "sModelNme")), 
                    ""
                    ));
        }
...
//        btnUpdate.setVisible(lbShow);
        ...
        if (lbShow) loRegistrationSelector.setFocus(txtField00);
    }
